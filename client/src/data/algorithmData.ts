...
#### 1. \`top, bottom, left, right\` (四面会移动的墙)
...
* **核心逻辑：**
    * 一开始房间很大（整个矩阵）。
    * 你贴着 **天花板** 爬完一行，天花板就 **塌下来一层** (\`top++\`)。
    * 你贴着 **右墙** 爬完一列，右墙就 **往左移一格** (\`right--\`)。
    * 直到天花板掉到了地板下面 (\`top > bottom\`)，或者左墙跑到了右墙右边 (\`left > right\`)，房间没了，游戏结束。

---

### 第二阶段：动作拆解与"交叉角"真相 (Action & Boundaries) —— 逻辑视角
...
* **关于角：** 当你走到右上角时，你把它装进包里了。
* **处理冲突：** 此时 \`top\` 变成了 1。下一轮向下走从 \`top\` (也就是 1) 开始！**所以右上角根本不会被第二步重复访问**。
...
* **避坑：** 此时 \`right\` 减小了。下一步向左走时，是从新的 \`right\` 开始的，所以右下角也不会重复。

#### 3. 向左走 (收割地板) —— **这里的 \`if\` 是真正的救命稻草！**

\`\`\`java
if (top <= bottom) { // 天花板压过地板了吗？
...
* **你担心的"交叉/重复"全靠这一行解决！**
* **场景模拟：** 假设矩阵只有 **一行** \`[1, 2, 3]\`。
    * **Step 1 (向右)：** 打印 \`1, 2, 3\`。\`top\` 变成 1。此时 \`top (1) > bottom (0)\`。
    * **Step 3 (向左)：** **如果没有这个 \`if\`**，程序会去打印 \`matrix[bottom][...]\`，也就是第 0 行！结果就是 \`1, 2, 3\` 又被打印了一遍！

* **结论：** 这个 \`if\` 是 **防"回头草"的**。如果天花板和地板已经重合甚至错开了，说明这一行已经被 Step 1 吃掉了，Step 3 必须闭嘴。

#### 4. 向上走 (收割左墙) —— 同样的道理

\`\`\`java
if (left <= right) { // 左墙跑到右墙外了吗？
...
#### 1. 为什么 \`while\` 里检查了，里面还要检查？

* **代码：** \`while (top <= bottom && left <= right)\`
* **疑问：** 大门口不是查过保安了吗？为什么进屋（Step 3 和 4）还要查身份证？
* **透视：** \`while\` 只是保证 **"刚进循环这一轮"** 是合法的。但是在 Step 1 (\`top++\`) 和 Step 2 (\`right--\`) 执行后，**局势变了！** 房间可能在这一瞬间已经塌了。Step 3 和 Step 4 必须根据 **最新的局势** 来判断是否继续。

#### 2. 角落的坐标到底是谁负责？

* 这是一种 **"接力棒"** 机制。
* **左上角：** 只有 Step 1 (向右) 会碰它。
* **右上角：** Step 1 会访问它作为 **终点**。Step 2 从它的 **下一个位置**（下一行）开始作为 **起点**。
* **总结：** 通过 \`top++\`, \`right--\` 等操作，我们把已经访问过的行/列彻底"移出"了可视范围。**下一棒的选手根本看不到上一棒跑过的角**。
