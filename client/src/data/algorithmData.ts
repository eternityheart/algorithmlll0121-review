...
\`\`\``
      },
      {
        id: "bs-3",
        title: "3. 找到两个边界",
        content: `### 第一阶段：变量选角 (The Casting Call) —— 为什么要加个 \`result\`？
...
- **场景：** 当数组只有两个元素，比如 \`[3, 1]\`。
  ...
  - 此时 \`nums[mid]\` (3) 等于 \`nums[left]\` (3)。`
      },
      {
        id: "bs-5",
        title: "5. 旋转数组找最小的（大胆）",
        content: `#### \`while (left < right)\` —— "只剩一人"原则
...
| **返回值**   | \`mid\`                    | \`nums[left]\`                           |`
      },
      {
        id: "bs-6",
        title: "6. 两个数组找中位数",
        content: `这段代码变量虽多，但逻辑非常严密。来，**"一句话变量说明书"**奉上：
...
1. **数组是类似一排房间**，门牌号是 \`0\` 到 \`N-1\`。`
      },
      {
        id: "bt-1",
        title: "1. 全排列",
        content: `**我们是在模拟一个"填空"的过程**。 我们要把 3 个空位填满，手里有 \`[1, 2, 3]\` 三张卡片。
...
3. 我得有个大相册，每次填满 3 格，就拍张照存进去。\`List<List<Integer>> result\``
      },
      {
        id: "bt-2",
        title: "2. 子集",
        content: `**进门先拍照，** (res.add first) **绝不走回头。** 
...
**最关键的一点：我接下来该从哪里开始挑？**（因为子集 \`[1, 2]\` 和 \`[2, 1]\` 是一样的，我们要避免回头看，防止重复，所以必须有一个变量来控制"只往后看"）。`
      },
      {
        id: "bt-3",
        title: "3. 电话字符",
        content: `**必须要有个"手指头" (Index)**：我们需要知道现在处理到 "23" 里的哪一位了。是刚开始按 '2'，还是按完了 '2' 该按 '3' 了？所以需要一个 \`int index\`。
...
### 🧐 研究生级深度点拨 (The "Gotchas") —— 主要病灶

\`\`\`java
List<String> res = new ArrayList<>();
// 【边界守卫】如果是空字符串，直接返回空列表，别让它进递归
...
- \`deleteChar\` ❌ -> \`deleteCharAt\` ✅ (StringBuilder 的标准方法名)`
      },
      {
        id: "bt-4",
        title: "4. 组合总和问题",
        content: `用个排序然后就是target是这里需要注意的一直是剩的目标大小。`
      },
      {
        id: "bt-5",
        title: "5. 括号匹配",
...
#### 3. \`isValid\` (安检员)`
  ...
  },
  {
    ...
    sections: [
      {
        id: "gr-1",
        title: "1. 股票、跳跃、步长、最远距离",
        content: `#### 1. 隐藏主角 \`int[] last\` (终点地图)
    ...
    sections: [
      {
        id: "dp-1",
        title: "字符分割思想",
        content: `### 核心心法：把索引看作"缝隙" (Fence Posts)

很多同学晕，是因为把 \`0, 1, 2\` 看作是字符本身。 **请把索引看作是字符之间的"缝隙"！**

\`\`\`
字符串： L E E T C O D E
缝隙(索引)：0 1 2 3 4 5 6 7 8
\`\`\`
...
- 切点必须在终点 \`i\` 的左边，所以 \`j < i\`。`
  ...
  },
  {
    id: "techniques",
    title: "技巧专题",
    tagClass: "tag-technique",
    icon: "🛠️",
    sections: [
      {
        id: "tech-1",
        title: "1. 众数（摩尔投票法）",
        content: `#### 修正一：起跑线的公平性 (\`int i = 1\`)
...
- **现在的写法：** \`return can\`。
  - *哲学：* **不忘初心**。题目问的是"谁赢了"，而不是"赢家还剩几滴血"。在摩尔投票法里，剩下的 \`count\` 是多少并不重要（它不代表众数出现的总次数），重要的是 \`can\` 指向谁。`
      ...
      {
        id: "tech-3",
        title: "3. 处理相同元素相邻（荷兰国旗问题）",
        content: `### 1. 为什么和 \`p2\` (后面) 换完**不能走**？—— 警惕"未知盲盒"
...
> **往前换 (swap p0)：** 换来的一定是**老实人**（1），因为前面都是我们检查过的，**放心大胆往前走**。`
      },
      {
        id: "tech-4",
        title: "4. 下一个排列",
        content: `好！这位朋友，你拿出的这段 \`nextPermutation\`（下一个排列）代码，可是算法界里**"看似青铜，实则王者"**的经典案例。它没有复杂的递归，也没有额外的数据结构，全是精密的指针操作。
...
5. **结果**：\`[1, 3, 1, 2, 4, 7]\`。这是刚好比原数组大的下一个数。`
      ...
      {
        id: "tech-5",
        title: "5. 相遇问题（弗洛伊德龟兔赛跑）",
        content: `这又是一段**"披着羊皮的狼"**！代码看着短得可怜，不到 15 行，但它背后藏着算法界最令人拍案叫绝的思维转换——**弗洛伊德龟兔赛跑算法 (Floyd's Tortoise and Hare)**。

如果不拆解，你可能会盯着 \`nums[nums[fast]]\` 发呆：这是在玩俄罗斯套娃吗？

来，我们进**"代码解剖室"**。首先你要做一个思维大挪移：**别把这当数组，要把它当成一个链表（Linked List）**。
...
- **普通视角：** \`nums[0] = 3\` 意思是第 0 个格子里放着数字 3。
- **本算法视角：** \`0 -> 3\`。意思是你在 0 号房间，手里拿到的线索是"去 3 号房间"。
- **为什么有环？** 题目说数字在 \`1\` 到 \`n\` 之间，且有一个重复数字。这就意味着有两个不同的房间（索引），都指向了同一个房间（值）。一旦走进那个重复的房间，就像掉进了一个**黑洞循环**，再也走不出去了。
- **我们的任务：** 找到这个黑洞的入口（重复的那个数字）。

---

### 第一阶段：变量选角面试 (The Casting Call) —— 幼儿园视角

**核心任务：** 解释为什么要请一只乌龟和一只兔子。

#### 1. \`int slow\` (乌龟/慢指针)

- **幼儿园比喻：** 这是一个**"老实的孩子"**。
- **动作特征：** 每次只走一步。\`slow = nums[slow]\`。
- **为什么要设计它：** 它负责稳扎稳打。如果它是那个唯一的探路者，它最终会陷在那个循环里出不来，但它自己不知道哪一个是入口。

#### 2. \`int fast\` (兔子/快指针)

- **幼儿园比喻：** 这是一个**"急躁的熊孩子"**。
- **动作特征：** 每次跳两步。\`nums[fast]\` 是跳一步到了下一个房间，\`nums[...  ]\` 是再拿那个房间里的线索跳到下下个房间。即 \`fast = nums[nums[fast]]\`。
- **为什么要设计它：**
  - 如果没有兔子，乌龟自己在圈里转一辈子也不知道有圈。
  - 因为兔子跑得快，如果跑道是直的，兔子早就没影了（数组越界）。但正因为有**圈**（循环），兔子最终会从后面追上乌龟（套圈）。
  - **比喻：** 就像操场跑步，跑得快的人最终会和跑得慢的人相遇。

#### 3. 初始化 \`slow = nums[0]\`, \`fast = nums[0]\`

- **为什么从 \`nums[0]\` 开始？**
  - 因为题目保证数字是 \`1\` 到 \`n\`。这意味着 \`0\` 这个数字**永远不会作为值出现**。
  - 换句话说，**0 号房间永远是入口，绝对不在那个循环圈里面**。从这里出发最安全。

---

### 第二阶段：动作拆解与边界守卫 (Action & Boundaries) —— 逻辑视角

**核心任务：** 为什么要先相遇，再重置？这是最高深的地方。

#### 1. 第一阶段：赛跑与相遇 (Detecting the Cycle)

\`\`\`java
do {
    slow = nums[slow];           // 乌龟走1步
    fast = nums[nums[fast]];     // 兔子走2步
} while (slow != fast);
\`\`\`

- **为什么用 \`do-while\`？**
  - 一开始两人都在起点 \`nums[0]\`。如果用 \`while(slow != fast)\`，一开始就相等，循环直接不执行，代码就废了。必须先让他们跑起来。
- **发生了什么？**
  - 兔子先冲进循环圈，在里面转圈圈。
  - 乌龟后进循环圈。
  - 因为兔子速度是乌龟的 2 倍，数学证明它们**一定会在圈里的某一点相遇**。
- **相遇点是结果吗？** **不是！** 相遇点只是圈里的随便一个位置，根本不是那个"入口"（重复数字）。

#### 2. 第二阶段：重置与寻找入口 (Finding the Entrance)

\`\`\`java
slow = nums[0]; // 关键魔法：把乌龟拎回起跑线
while (slow != fast) {
    slow = nums[slow]; // 乌龟这次还是走1步
    fast = nums[fast]; // 注意！兔子这次也改走1步了！
}
\`\`\`

- **为什么要这么写（数学魔法）？**
  - 这是一个经典的数学推导结论：**当它们在圈内相遇后，把其中一个人扔回起点，另一个留在相遇点。然后两人都用"速度1"开始走，它们必然会在"环的入口"相撞。**
...
1. **映射关系：**
   ...
   - 3 -> 2
   - 2 -> 4
   - 4 -> **2** (这里！4 和 3 都指向了 2。2 就是那个入口/重复数)
   - **路径：** 0 -> 1 -> 3 -> **2** -> 4 -> **2** -> 4 ... (形成环：2-4-2-4)
2. **第一阶段（找相遇）：**
   - Start: S=1, F=1
   - Step 1: S=3, F=2 (F跳两步: 1->3->2)
   - Step 2: S=2, F=2 (S跳: 3->2; F跳: 2->4->2)。**相遇！** 停在 2。
3. **第二阶段（找入口）：**
   - S 回到起点 \`nums[0]\` = 1。 F 留在 2。
   - Step 1: S=3 (1->3), F=4 (2->4)。不相等。
   - Step 2: S=2 (3->2), F=2 (4->2)。**相等！**
   - **Return 2**。正确答案。`
      }
    ]
  }
...
export const getSectionById = (categoryId: string, sectionId: string): Section | undefined => {
